...
const Dashboard = () => {
  ...
  const getIntroOfPage = (label) => {
    ...
    if (label === 'Checked') {
      return "Checked messages count";
    }
    if (label === 'Unchecked') {
      return "Unchecked messages count";
    }
    if (label === 'Sun') {
      return 'Messages count on Sunday';
    }
    if (label === 'Mon') {
      return 'Messages count on Monday';
    }
    if (label === 'Tue') {
      return 'Messages count on Tuesday';
    }
    if (label === 'Wed') {
      return 'Messages count on Wednesday';
    }
    if (label === 'Thu') {
      return 'Messages count on Thursday';
    }
    if (label === 'Fri') {
      return 'Messages count on Friday';
    }
    if (label === 'Sat') {
      return 'Messages count on Saturday';
    }
    return '';
  };
        ...
        <Grid container spacing={3} justifyContent="flex-end">
          <Grid item xs={12}>
            {/* <Paper className={classes.fixedHeightPaper}> */}
              ...
              <Grid xs={4} sm={4} item className={classes.chart_container_items}>
                    ...
                    {chart_data_schedule ? chart_data_schedule.map((entry, index) => (
                      <TimelineItem key={index}>
                        ...
                        <TimelineOppositeContent fontSize={12} color="textSecondary">
                          {(entry.contents.length > 20 ) ? `${entry.contents.substr(0, 20)}...` : entry.contents.substr(0, 20)}
              ...
              <Grid xs={4} sm={4} item className={classes.chart_container_items}>
                <Grid xs={12} item className={classes.chart_container}>
                  ...
                  <ResponsiveContainer item className={classes.chart_item}>
                    <BarChart
                      width={500}
                      height={300}
