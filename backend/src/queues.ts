  ...
  try {
    ...
    if (count > 0) {
      schedules.map(async schedule => {
        ...
        logger.info(`Trigger scheduled for: ${schedule.contact.name}`);});
    }
  } catch (e: any) {
    Sentry.captureException(e);
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    Sentry.captureException(e);
    logger.info(`Error when trying to query schedule: ${schedule.id}`);  }
...
async function handleVerifyCampaigns(job) {
  /**
   * @todo
   * Implementar filtro de Campaigns
   */
  ...
  for (let campaign of campaigns) {
    try {
      ...
      logger.info(
        `Campaign sent to processing queue: Campaign=${campaign.id}, Initial Delay=${delay}`
      );
      campaignQueue.add(
        "ProcessCampaign",
        {
...
async function sleep(seconds) {
  ...
  return new Promise(resolve => {
    setTimeout(() => {
      logger.info(
        `Sleep of ${seconds} seconds done: ${moment().format(
          "HH:mm:ss"
        )}`
      );
      resolve(true);
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        for (let contact of contacts) {
          ...
          logger.info(
            `Record sent to the firing queue: Campaign=${campaign.id};Contact=${contact.name};delay=${delay}`          );
          index++;
...
async function handleDispatchCampaign(job) {
  try {
    ...
    logger.info(
      `Campaign firing requested: Campaign=${campaignId};Registration=${campaignShippingId}`    );
    ...
    logger.info(
      `Campaign sent to: Campaign=${campaignId};Contact=${campaignShipping.contact.name}`    );
  } catch (err: any) {
    Sentry.captureException(err);
...
async function handleLoginStatus(job) {
  ...
  for (let item of users) {
    try {
      ...
      logger.info(`User taken offline: ${item.id}`);    } catch (e: any) {
      Sentry.captureException(e);
    }
  }
}
